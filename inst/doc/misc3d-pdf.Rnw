...
by the \texttt{misc3d} R package. So far there are three
approaches available in \texttt{misc3d}, one that captures only the 
...
format.  Function \texttt{exportScene} is used to write out \texttt{misc3d} 
scenes in OFF and other textual format.
...
OFF files with color and transparancy (COFF).  
The OFF file produced by this function can then be read in
...
The \emph{Intermediate Data Text Format}\href{http://wiki.jmol.org/index.php?title=File_formats/3D_Objects}
{ (IDTF)}, is aother textual format that can be transformed into the U3D format.
The IDTF is superior to OFF in that color and transparency are supported.
\href{http://wiki.jmol.org/index.php?title=File_formats/3D_Objects}{The IDTF converter} can be used to produce U3D files. 

Color can be used to help distinguish multiple contour surfaces
as shown in Figure \ref{fig:nmix-color}. We need to export the contour surfaces
<<eval=false>>=
exportScene(conts, "nmix-color", "IDTF")
@ 
ane then convert it to U3d file.
\begin{figure}[h]
  \centering
...
For the IDTF format, we still need to figure out how to properly produce 
a single mesh surface with different colors like in the volcano example.
For both the IDTF and Asymptote, we have not yet figured out whether 
it is possible to have different colors on the two sides of a facet, 
or how to specify other material properties, which may well be possible.  

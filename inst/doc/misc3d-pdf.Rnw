...
For the IDTF format, we figured out a way to produce a single mesh surface 
...
we have not yet figured out whether it is possible to have different colors 
on the two sides of a facet, or how to specify other material properties, 
